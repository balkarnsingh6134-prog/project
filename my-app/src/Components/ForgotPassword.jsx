...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:5555/user/forgot-password", { email });
      
      if (response.data.success) {
        toast.success("Reset link sent! Please check your email.");
        navigate("/login");
      } else {
        toast.error(response.data.message || "Failed to send link");
      }
    } catch (error) {
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="text-center mt-3">
                  <span onClick={() => navigate("/login")} style={{ cursor: "pointer", color: "#06c4dd", fontWeight: "500", textDecoration: "underline" }}>
                    Back to Login
                  </span>
                </div>
              </form>
